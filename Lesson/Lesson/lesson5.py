'''
    Ký tự và chuỗi
        ...
        String: Chuỗi, Xâu
        Ký tự: các chữ cái, chữ số, dấu câu,...
'''
# Cắt chuỗi cơ bản
s = 'Python Core'
'''
...
'''
    Iterating: Duyệt các ký tự trong chuỗi:để làm việc với từng ký tự trong chuỗi, ta có thể 
    dùng vòng lặp for để duyệt theo chỉ số hoặc duyệt trực tiếp từng ký tự
'''

'Ví dụ: Đếm xem trong chuỗi vừa nhập từ bàn phím có bao nhiêu ký tự số'

# Cách 1. Dùng chỉ số để duyệt - Duyệt gián tiếp -> Dùng khi cần quan tâm chỉ số
''' 
...
'Lấy tất cả các vị trí của ký tự "a" trong chuỗi s được nhập '

s0 = input('Nhập chuỗi s = ')
...
''' 
    Các hàm phổ biến với chuỗi
        s.upper(): In hoa
        s.lower(): In thường
        s.title(): Viết hoa các chữ cái đầu các từ và viết thường các ký tự khác trong chuỗi
        s.strip(): Xóa bỏ khoảng trắng ở đầu và cuối chuỗi s
        s.lstrip(): Xóa bỏ tất cả khoảng trắng ở đầu chuỗi s
...
''' 
    Tìm kiếm và thay thế trong chuỗi
        ...
        s.find(sub): Trả ra chỉ số (vị trí) đầu tiên bắt đầu xuất hiện 
        s.rfind(sub): Trả ra chỉ số cuối cùng bắt đầu xuất hiện
...
'''
ab = 'Python is amazing'
print(ab.find('on'))
print(ab.find('i'))
print(ab.find('P'))
print(ab.find(' '))
print(ab.startswith("Py"))
print(ab.endswith("ing"))
print(ab.replace('is'), 'are')
